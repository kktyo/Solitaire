...
async function typeInto(page: Page, id: string, label: string, value: string) {
  const node = flutterId(page, id).or(page.getByLabel(label, { exact: true }));
...
test.describe('Flutter Web', () => {
  test('register reaches home', async ({ page }) => {
    ...
    await expect(flutterId(page, 'register-email').or(page.getByLabel('メール'))).toBeVisible({ timeout: 60_000 });
    ...
    await flutterId(page, 'register-submit').or(page.getByRole('button', { name: '登録' })).first().click();
